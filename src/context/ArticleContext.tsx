...
import {
  ...
  fetchArticleByCategory as fetchArticleByCategoryService,
...
interface ArticleContextData {
  ...
  fetchArticleByCategory: (category: string) => Promise<Article[]>
}
...
const ArticleProvider: React.FC<Props> = ({ children }) => {
  ...
  const fetchArticleByCategory = async (
    ...
    try {
      const result = await fetchArticleByCategoryService(category)
      return result
    } catch (error) {
      handleError(error)
      throw error
