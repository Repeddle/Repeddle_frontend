...
const NewProduct = () => {
  ...
  useEffect(() => {
    ...
    fieldsToCheck.forEach((field) => {
      const value = input[field as keyof InputData];
      if (typeof value === "string") {
        const found = checkRestricted(value);
        if (found.length > 0) {
          newWarnings[field] = found;
        }
      }
    });

    // Check tags array as well
    const tagWarnings = tags.flatMap((tag) => checkRestricted(tag));
    if (tagWarnings.length > 0) {
      newWarnings["tags"] = [...new Set(tagWarnings)];
    }

    setModerationWarnings(newWarnings);
  }, [input, tags, checkRestricted]);
