...
const Categories = () => {
  ...
  const handleDeleteItem = (subCateIndex: number, itemIndex: number) => {
    const newSubCategories = subCategories.map((subCate, i) => {
      if (i === subCateIndex) {
        const updatedItems = subCate.items.filter(
          (_, idx) => idx !== itemIndex
        );
  ...
  const deleteHandler = async (cate: ICategory) => {
    ...
    if (res) {
      ...
      addNotification(error ? error : "category has been deleted");
    }
  };
  ...
  const submitHandler = async () => {
    ...
    if (!editCat) {
      ...
      if (res) {
        ...
        addNotification(error ? error : "Failed to create categories");
      }
    } else {
      ...
      if (res) {
        ...
        addNotification(error ? error : "Failed to update categories");
      }
    }
